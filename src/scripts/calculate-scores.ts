...
/**
 * 통화별 기본 비용 점수 (절대 물가 수준 반영, v2.1 보정)
 *
 * 한국인 여행자 관점에서 저렴할수록 높은 점수.
 * 단, "비싼 나라"에도 최소 4.5점을 보장하여
 * 물가만으로 순위가 결정되지 않도록 범위를 압축.
 *
 * 범위: 4.5 ~ 8.0 (이전 3.0 ~ 8.5 → 최대 격차 5.5에서 3.5로 축소)
 * 종합 점수 영향: 최대 0.88점 차이 (이전 1.38점)
 */
const CURRENCY_BASE_COST: Record<string, number> = {
  ...
  IDR: 7.5,  // 인도네시아 — 저렴
  MYR: 7.0,  // 말레이시아 — 저렴
  SGD: 5.5,  // 싱가포르 — 비쌈 (4.5→5.5)
  KHR: 8.0,  // 캄보디아 — 매우 저렴
  LAK: 8.0,  // 라오스 — 매우 저렴 (8.5→8.0)
  MMK: 8.0,  // 미얀마 — 매우 저렴 (8.5→8.0)
  // 중동
  AED: 5.0,  // UAE — 비쌈 (4.5→5.0)
  TRY: 7.5,  // 터키 — 저렴 (리라 약세)
  // 유럽 — 바닥 올림: 비싸도 여행 가치 고려
  EUR: 5.0,  // 유로존 (4.0→5.0)
  GBP: 4.5,  // 영국 (3.5→4.5)
  CZK: 6.0,  // 체코 (5.5→6.0)
  CHF: 4.5,  // 스위스 (3.0→4.5)
  HUF: 6.5,  // 헝가리 (6.0→6.5)
  PLN: 6.0,  // 폴란드 (5.5→6.0)
  ISK: 4.5,  // 아이슬란드 (3.5→4.5)
  NOK: 4.5,  // 노르웨이 (3.5→4.5)
  SEK: 5.0,  // 스웨덴 (4.0→5.0)
  DKK: 5.0,  // 덴마크 (4.0→5.0)
  HRK: 5.5,  // 크로아티아 (5.0→5.5)
  // 미주
  USD: 5.0,  // 미국 (4.5→5.0)
  CAD: 5.0,  // 캐나다 (4.5→5.0)
  MXN: 7.0,  // 멕시코 — 저렴
